...
def confirmar_aposta():
    ...
    saldo = executar_query_fetchall("SELECT pontos FROM usuarios WHERE id = %s", (id_ganhador,))
    if not saldo:
        return jsonify({'error': 'Ganhador não encontrado'}), 404
    
    saldo_atual = saldo[0][0]
    novo_saldo = saldo_atual + valor_inicial
    
    # Atualizar pontos do ganhador
    sucesso = executar_query_commit("UPDATE usuarios SET pontos = %s WHERE id = %s", (novo_saldo, id_ganhador))
    ...
    if sucesso:
        ...
        return jsonify({
            'message': f'Jogador {id_ganhador} ganhou {valor_inicial} pontos',
